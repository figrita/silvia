...
registerNode({
    ...
    for(float i = -1.0; i <= 1.0; i += 1.0) {
        for(float j = -1.0; j <= 1.0; j += 1.0) {
            if(abs(i) + abs(j) <= 1.0) { // Cross pattern
                ...
                // Dilate: take maximum value
                float sampleBrightness = dot(texSample.rgb, vec3(0.299, 0.587, 0.114));
                float maxBrightness = dot(maxValue.rgb, vec3(0.299, 0.587, 0.114));
                
                if(sampleBrightness > maxBrightness) {
                    maxValue = texSample;
                }
            }
        }
    ...
    float dilatedBrightness = dot(maxValue.rgb, vec3(0.299, 0.587, 0.114));
    
    if(dilatedBrightness - originalBrightness > ${threshold}) {
        return mix(original, maxValue, ${intensity});
    } else {
        return original;
    }
}`
