...
registerNode({
    ...
    icon: '🗺️',
    label: 'Mandelbrot',
    tooltip: 'Renders the famous Mandelbrot fractal. Renders as a gradient, or use as a texture mapping effect in map mode.',
    ...
    input: {
        ...
        'centerX': {
            label: 'Center X',
            type: 'float',
            control: {default: -0.5, min: -2.0, max: 2.0, step: 0.001, unit: '⬓'}
        },
        'centerY': {
            label: 'Center Y',
            type: 'float',
    ...
    _createShader(type, source){
        ...
        if(!gl.getShaderParameter(shader, gl.COMPILE_STATUS)){
            console.error(`Mandelbrot Preview Shader Error: ${gl.getShaderInfoLog(shader)}`)
    ...
    _createProgram(vertexShader, fragmentShader){
        ...
        if(!gl.getProgramParameter(program, gl.LINK_STATUS)){
            console.error(`Mandelbrot Preview Program Link Error: ${gl.getProgramInfoLog(program)}`)
