 ...
 * Pass the same Float32Array twice for mono sources — the file will be
 * hard-panned mono but is still a valid stereo WAV.
 */
...
registerNode({
    ...
    runtimeState: {
        ...
        audioCompileError: null,  // non-null → displayed in statusLine
        // WAV path
        wavChunks: null,
            ...
            workletNode.port.onmessage = (e) => {
                ...
                if(m.type === 'record-data'){
                    if(this.runtimeState.wavChunks){
                        // ch1 may be null from hosts that hand us a mono
                        // output bus; replicate ch0 so the stereo file
                        // stays valid either way.
                        this.runtimeState.wavChunks.push({
                            ch0: m.samples || m.ch0,
                            ch1: m.ch1 || m.samples || m.ch0
                        })
                    }
    ...
    _finalizeWavRecording(){
        ...
        const blob = encodeWAV(ch0, ch1, this.runtimeState.wavSampleRate)
    ...
    _updateStatusLine(){
        ...
        // Compile errors take precedence over recording state so the
        // user notices a broken patch instead of assuming everything
        // is fine (previous program keeps playing silently).
        if(this.runtimeState.audioCompileError){
            this.elements.statusLine.textContent = `⚠ ${this.runtimeState.audioCompileError}`
            this.elements.statusLine.style.color = '#ffb020'
            this.elements.statusLine.title = this.runtimeState.audioCompileError
            this.elements.recBtn.textContent = '● Rec'
            this.elements.recBtn.classList.remove('recording')
            return
        }
        this.elements.statusLine.title = ''
        if(this.runtimeState.isRecording){
    ...
    /**
     * Display a compile error on the sink node. Called by audioRuntime
     * when compileGraph throws an AudioCompileError; cleared (with
     * null) when the next compile succeeds. The previous program keeps
     * playing regardless — this is a visibility hook, not a stop.
     */
    _setAudioCompileError(message){
        this.runtimeState.audioCompileError = message || null
        this._updateStatusLine()
    },
